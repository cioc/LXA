...
def SignatureDifference (sig1, sig2, outfile):	# this finds the best alignments between affixes of a signature, and also gives a measure of the similarity.
	#fix for error on line "bestvalue = list3[0]"  
	if (len(sig2) > 0 and sig2[0] == '-' and sig2[1] == '-'):
			sig2 = sig2[2:]
	if (len(sig1) > 0 and sig1[0] == '-' and sig1[1] == '-'):
			sig2 = sig1[2:]
	...
	if reversedFlag:
		#fix; added zero to fix unpacking problem
		return (0, AlignedList2,AlignedList1)
...
#----------------------------------------------------------------------------------------------------------------------------#
	...
	for sig, stemlist in SortedListOfSignatures:
		if len(stemlist) < stemcountcutoff:
			continue;
...
#----------------------------------------------------------------------------------------------------------------------------#
def MakeSignatures(StemToWord, StemToSig, FindSuffixesFlag, outfile, NoLengthLimitFlag = False): # Signatures is a map from the signature to a set of its stems.
#----------------------------------------------------------------------------------------------------------------------------#
	# Signatures is a map: its keys are signatures. Its values are *sets* of stems. 
	# StemToWord is a map; its keys are stems.      Its values are *sets* of words.
	# WordToSig is a Map. its keys are words.       Its values are *lists* of signatures
	...
	for stem in StemToWord.keys():
		...
		if len(thissig) == 0:
			print "no length signature line 1161", stem
...
#----------------------------------------------------------------------------------------------------------------------------#
	...
	#print "adding" , shift, "to", sig_target
	for stem in stemset:
		thesewords = []
		if not stem.endswith(shift):
			continue
